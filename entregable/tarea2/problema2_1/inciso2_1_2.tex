...
\afterstatement
    ...
    y por lo tanto no hay nada que demostrar. Análogamente ocurre con $t=1$.\pn
    
    Entonces concentrémonos en el caso donde $t\in (0, 1)$.\pn
    ...
    Sean entonces $p = \frac{1}{1-t}$ y $q = \frac{1}{t}$. Tenemos que\pn
    ...
    Veamos que $e^{-a(1-t) X_1}$ pertenece a $L_p$.\pn
    ...
    [\ref{problema2_1:inciso1}]. Con esto hemos demostrado que $e^{-a(1-t) X_1}$ pertenece a $L_p$.\pn
    
    De manera análoga se puede demostrar que $e^{-b(t) X_1}$ pertenece a $L_q$.\pn
    
    Ahora que tenemos todas las hipótesis para la desigualdad de Hölder, basta aplicarla.\pn
    ...
    Que es lo que necesitabamos mostrar para probar que $g$ es $\log$-convexa.\pn
    
    Ahora supongamos que $\mw(X_1 = -1) > 0$.\pn
    ...
    $\lambda$ crece, tenemos que $g(\lambda)$ también lo hace.\pn
    
    Probemos ahora que $(\frac{1}{g}) \circ f (s) = s$ para toda $s \in (0,1)$.\pn
    ...
    convexa creciente tenemos que $g$ es convexa.\pn
    ...
    $g$ está definida sobre todo $\R$, que es abierto, y por lo tanto $g$ es continua.\pn
    ...
    tenemos entonces que $\frac{1}{s} \leq g(\lambda_0)$.\pn
    ...
    $g(\lambda_0) = \frac{1}{s}$.\pn
    
    De esto último, tenemos que $g(\lambda_0) = g(f(s)) = \frac{1}{s}$ y por lo tanto \\
    $\frac{1}{g(f(s))} = (\frac{1}{g}) \circ f (s) = s$ y con esto terminamos la demostración que buscábamos.\pn
    
    Ahora supongamos que $g(\lambda) > 1$.\pn
    ...
    $e^{-\lambda S_n(A_n)} \leq e^{\lambda k}$.\pn
    ...
    Donde en el último paso se usa la hipótesis de que $g(\lambda) > 1$.\pn
    ...
    es perfectamente válida cuando $T_k = \infty$.\pn
    ...
    Como $T_k^n$ no rebasa a $T_k$, entonces, por la parte anterior de este ejercicio $M_{T_k^n}$ es acotada.\pn
    ...
    De donde $e^{-f(s) k} = \E(s^{T_k})$. Como queríamos demostrar.\pn
    
    Ahora, si $T_k = \infty$, ya que $s \in (0,1)$, entonces $s^(T_k) = 0$.\pn
    ...
    Es decir $f(s) = \inf \{ \lambda > 0 : \frac{1}{s} < g(s)\} = \inf \emptyset = \infty$.\pn
